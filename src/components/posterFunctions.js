...
export function calculatePosterWidth(area, boot) {
	...
	waitForElement('section .poster').then(() => {
		document.querySelectorAll('section .poster').forEach((poster) => {
			poster.style.width = `${posterWidth}px`;

			// Set transition property afterwards fo the initial width
			if (boot)
				setTimeout(() => poster.style.transitionProperty = 'all', 100);
		});
	});
}

export function getPoster(media, server, platform) {
	let poster = media.images.filter(i => i.coverType === 'poster')[0]?.url
	if (poster == undefined)
		poster = media.images[0]?.url
	if (poster == undefined)
		poster =
			platform === 'radarr' ?
		...
		poster = `http${server.ssl ? 's' : ''}://${server.url}:${server.port}${poster}`
	return poster
}